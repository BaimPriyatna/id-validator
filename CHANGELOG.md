...
- `phone` (`id-validator-id`) — accepts local Indonesian input (`0812...`, `628...`, with separators) before normalizing to `+62` E.164. `validate()` applies the same input-safety guard as `isValid()` (fixed: the wrapper previously only guarded `isValid()`, so `validate()` could still throw a raw `TypeError` on a non-string input like a number or object instead of returning a proper `ValidationResult`).
...
- 207 tests passing across 10 test files (vitest).
